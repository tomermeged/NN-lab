...
def max_pool_2x2(inputT, strideS, frac_ratio=None, kernelS=2): # S=size; inputT shape: [images, W, H, Channels]
    ...
    """
    if frac_ratio == None or frac_ratio[0] == CM.OFF:
...
class maxPoolLayer():  # S=size; inputT shape: [images, W, H, Channels]
    ...
    def __init__(self, inputT, layerSize, kernelSize=2, stride=2, frac_ratio=None):
        self.type = "max_pool"
        self.inputT = inputT
        self.kernelSize = kernelSize
        self.stride = stride
    ...
    def create_layer(self):
        if self.frac_ratio == None or self.frac_ratio[0] == CM.OFF:
            ...
            self.outputT, _, _ = tf.nn.fractional_max_pool(self.inputT,
                                            pooling_ratio=[1.0, self.frac_ratio[1], self.frac_ratio[2], 1.0],
                                            pseudo_random=True)
        return self.outputT
